...
const SetChunkSize = 100_000_000 // 집합 생성 시 한 번에 처리할 집합 수
...
func main() {
	...
	if planA {
		...
		err, multiSets := processor.ProcessSingleSets(allSets)
		if err != nil {
			logf("단일 집합 처리 실패: %v\n", err)
			return
		} else {
		...
		testResult.SingleSets += len(allSets) - len(multiSets)
		...
		logf("  %d/%d 집합 처리 완료 (단일: %v, 다중: %v)\n",
			len(allSets), len(allSets), singleTime, multiTime)
